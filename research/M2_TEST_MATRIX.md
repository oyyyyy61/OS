...
Status: v3 item-8 accepted; aggregate M2 open, 2026-07-26.
...
items 1-7. Campaign02 closes item 8; aggregate M2 remains open.
...
closes item 8 only; aggregate M2 and performance claims remain open.

The final column separates the remaining aggregate acceptance action from
non-blocking strengthening or later-stage policy evidence. Only the explicit
cross-evidence audit and acceptance decision block aggregate M2 under the
current `STAGE_GATES.md` contract; the live-workload additions do not.

| Gate or evidence dimension | Direct M2 evidence | Remaining aggregate action or non-blocking later-stage evidence |
| --- | --- | --- |
| Canonical identities | `test_domain.py`, repository import-boundary AST check, the diagnostic contract, v3 run10, 59 fresh calibration processes, and 20 fresh formal holdouts bound live GPU allocation generations and exact CPU allocation records | none for item 8; aggregate M2 cross-evidence audit remains |
...
| Provenance | the 59-run calibration and 20-run formal bundles bind their marker-only execution HEADs, DAGKV/vLLM snapshots, implementation manifest, runtime fingerprint, mapped `libcuda`, audited 239-to-250 dependency boundary, and NVIDIA 580.173.02 bundle; both complete replays passed | aggregate M2 cross-evidence audit remains |
| Item-8 aggregate acceptance | accepted; campaign02 has 20/20 holdout-pass manifests, one exact 42-record journal, `M2_ITEM8_ACCEPTANCE_MANIFEST.json`, and an independently replayed bundle seal | none for item 8; aggregate M2 remains open |
...
record; the sealed 20-run aggregate closes item 8. `STAGE_GATES.md` remains the
aggregate M2 acceptance authority.
...
current-vLLM CPU tests. These CPU results remain separate from the completed
20-run CUDA formal cohort and its independently replayed evidence.
