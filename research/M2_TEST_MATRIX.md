...
| Transfer metadata integrity | component tests cover exact terminal bytes/digests, failed-target no-publish, and cleanup; run06 pilot recorded four exact `2,359,296`-byte DMA terminal pairs with one canonical digest | all 59 calibration and 20 formal holdout traces under one frozen source/runtime fingerprint |
| GPU re-admission path | runtime and independent-ledger rejection of direct CPU-only publication, plus successful H2D retry; run06 pilot observed fresh GPU target identities and CPU generations 1 then 2 | live adapter API isolation and v2 cohort evidence |
...
| No-DMA prefix control | run06 observed 16 GPU-hit tokens, zero extra DMA, exact `G=B1=B2` raw logits, and `A1-G max_abs=0.109375` | repeat across the frozen 59+20 cohorts |
| Output correctness | run06 decoded token 932 in all five phases and recorded exact A1/A2 and G/B1/B2 vectors | 59 independent fresh-process calibrations at `atol=0.125, rtol=0`; then 20 new frozen formal holdouts with exact within-path repeats and exact token equality |
| Provenance | run06 hashes the clean DAGKV tree, complete dirty-vLLM patch/untracked set, 16 model files, five weight shards, six native extensions, dependencies, environment, and every raw artifact | repeat one fingerprint across the complete cohorts and freeze aggregate inputs |
